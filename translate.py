...
def main():
    ...
    for line in addone(open(opt.src)):
        ...
        if opt.encode:
          encodings = translator.translate(srcBatch, tgtBatch, eout=True)
          h_list.append(encodings[0].data)
          c_list.append(encodings[1].data)
          srcBatch, tgtBatch = [], []
          continue
    ...
    if opt.encode:
      encodings_list = [torch.cat(h_list,1),torch.cat(c_list,1)]
      torch.save(encodings_list, open("encodings.pt",'wb'))
      return
