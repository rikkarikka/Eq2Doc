    ...
    for x in s:
      if x[4] != "O":
        ...
        if x[4]!=last:
          if last not in d:
            d[last] = {}
          thisner = " ".join(thisner)
          if thisner not in d[last]:
            d[last][thisner] = []
          d[last][thisner].append(i)
          thisner = []
      last = x[4]
  return d
...
def nertext(ner,nerd,txt):
  ...
  '''
  if len(nertxt)!=len(txt2):
    for x in d:
      for i,v in enumerate(d[x]):
        rstring = x+"_"+str(i)
        txt = txt.replace(v,rstring)
    return txt
  '''
...
def doNER(ner,srl,nerd):
  ...
  return srl,d #" ".join(nertxt),d
    

def main():
  bad = 0
  walk = os.walk('data/wiki/simple70')
  ...
  while files:
    ...
    for f in plaintexts:
      ...
      with open("data/wiki/simpleNER/"+f[:-5]+".conll") as g:
        #print(f)
      ...
      while i < min(len(srl),len(txt_)):
        if srl[i] != []:
          ...
          if std == []:
            ...
            print("FAILURE")
            exit()
          asrl,nerd = doNER(ner[i],std,nerd)
          src.append(asrl)
        i+=1
      try:
  ...
  print("Failed: ",bad)
  with open("pickles/anonWikiData-2.21.pickle",'wb') as f:
